...
async function fetchData() {
    ...
    try {
        ...
        const response = await fetch("https://mspfa.com/", {
            ...
            body: new URLSearchParams({ do: "story", s: 51062 }),
        });
...
async function renderPage(pages) {
    ...
    if(nextPage !== undefined) {
        ...
        link.href = `drinkingproblem.html?p=${pageNum + 1}`
    ...
    logLink.className = "comicLinks"
    logLink.href = `DPLog.html`
    logLink.textContent = `Story Log`;
    const goBack = document.createElement('a');
    goBack.className = "comicLinks";
    goBack.href = `drinkingproblem.html?p=${pageNum - 1}`;
