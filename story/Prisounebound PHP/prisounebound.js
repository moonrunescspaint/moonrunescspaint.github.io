...
async function renderPage(pages, commentBoxes, commands) {
    ...
    let pageBody = page.b
    const addSpan = (parentId, content, isNode = false) => {
    const span = document.createElement('span');
    isNode ? span.append(content) : span.textContent = content;
    document.getElementById(parentId).replaceChildren(span);
};

    addSpan("command", page.c);
    addSpan("content", parseBBCode(pageBody), true);
    ...
    if(nextPage !== undefined) {
        ...
        link.href = `prisounebound.php?p=${pageNum + 1}`
    ...
    const startOver = Object.assign(document.createElement('a'), {
        href: 'prisounebound.php?p=1',
...
function comments(commentBoxes) {
    ...
    commentIndicators(comments)

    for (let i = 0; i < comments.length; i++) {
        const commentBox = comments[i];
        const posterId = postNums[i]
        ...
        for (let j = 0; j < lines.length; j++) {
            ...
            backlinks(line, posterId, replyId, postNums)
...
function backlinks(line, posterId, replyId, postNums){

    if (line.includes(replyId) && replyId == postNums) {
        console.log(true)
    }
...
function commentIndicators(comments) {
    
    const firstComment = comments[0]

    const commentHeart = Object.assign(document.createElement('img'), {
        src: 'https://file.garden/ZXENUjtcLzaPWMyp/website%20stuff/firstcomment.png',
        className: 'commentHeart',
        title: 'First Commentor!',
        imageSmoothingEnabled: false
    });
    
        if (firstComment) {
            firstComment.classList.add('firstComment');

            const firstCommentInfo = document.querySelector('span.u')
            firstCommentInfo.append(commentHeart)
        }


    //To indicate the latest comment.
    const recentComment = comments[comments.length - 1]
        if (recentComment) {
            recentComment.classList.add('recent');
        }


    //For appending a reply id of a comment to the form — so that a user can reply to that post with the corresponding id.
    const postNumsId = document.querySelectorAll('span.postNum');
    const form = document.getElementById('formArea');

    postNumsId.forEach(post => {
        post.addEventListener('click', () => {
        form.value += `>>${post.textContent}\n`;
        });
    });
...
function removeRecent() {
  const recentComment = document.querySelector("div.commentBox.recent");
  recentComment.classList.remove("recent");
  recentComment.style.boxShadow = 'none'

}

//Page autoreload for commments.
let reloading;
...
function handleSuccess(token) {
    ...
    document.addEventListener('DOMContentLoaded', () => {     
      const widgetId = turnstile.render("#formSection", {
          sitekey: "0x4AAAAAACCUZuarE0Z0TbdX",
